...
function apms_get_text($str) {
	...
	$str = preg_replace("/{지도\:([^}]*)}/is", "", $str);
	$str = preg_replace("/{이미지\:([^}]*)}/is", "", $str);
	$str = preg_replace("/{동영상\:([^}]*)}/is", "", $str);
	$str = preg_replace("/{아이콘\:([^}]*)}/is", "", $str);
	$str = preg_replace("/{이모티콘\:([^}]*)}/is", "", $str);
...
function apms_fa($str){
	$str = ($str) ? preg_replace_callback("/{아이콘\:([^}]*)}/is", "apms_callback_icon", $str) : $str;
	return $str;
}
...
function apms_emo($str){
	$str = preg_replace_callback("/{이모티콘\:([^}]*)}/is", "apms_callback_emoticon", $str); // Emoticon 
	return $str;
}
...
function apms_video($vid) {
	...
	if($video['type'] == "file") { //JWPLAYER
		...
		if($video['type'] == "youtube") { //유튜브
			...
			$autoplay = ($autoplayvideo || $video['auto']) ? '&autoPlay=1' : '';
			$show = '<iframe width="'.$video['width'].'" height="'.$video['height'].'" src="http://videofarm.daum.net/controller/video/viewer/Video.html?vid='.$video['rid'].'&play_loc=undefined'.$autoplay.'&wmode=opaque" frameborder="0" scrolling="no"'.$fullscreen.'></iframe>';
...
function apms_callback_map($m) {
	return apms_google_map($m[1]);
}

function apms_callback_video($m) {
	return apms_video($m[1]);
}
...
function apms_callback_icon($m) {
	return apms_icon($m[1]);
}

function apms_callback_emoticon($m) {
	return apms_emoticon($m[1], '');
}
...
function apms_content($explan) {
	...
	$explan = preg_replace_callback("/{지도\:([^}]*)}/is", "apms_callback_map", $explan); // Google Map
	$explan = preg_replace_callback("/{동영상\:([^}]*)}/is", "apms_callback_video", $explan); // Video
	$explan = preg_replace_callback("/\[soundcloud([^\]]*)\]/is", "apms_callback_soundcloud", $explan); // SoundCloud
	$explan = preg_replace_callback("/{아이콘\:([^}]*)}/is", "apms_callback_icon", $explan); // FA Icon
	$explan = preg_replace_callback("/{이모티콘\:([^}]*)}/is", "apms_callback_emoticon", $explan); // Emoticon 
...
function apms_img_content($content) {
	...
	if($matches[1][0]) {
		...
		if(preg_match_all("/{동영상\:([^}]*)}/is", $content, $match)) {
...
function apms_board_rows($arr) {
	...
	if(!$bo_table || count($board_cnt) > 1 || $except) { //복수
		...
		$sql_common = "from {$g5['board_new_table']} where $sql_mode2 $sql_mode3 $sql_find $sql_term $sql_main $sql_image $sql_mb";
		if($page > 1) {
		...
		$tmp_write_table = $g5['write_prefix'] . $bo_table;
		$sql_common = "from $tmp_write_table where wr_is_comment = '{$sql_mode1}' $sql_mode3 $sca_query $sql_term $sql_main $sql_image $sql_mb";
		if($page > 1) {
			$total = sql_fetch("select count(*) as cnt $sql_common ", false);
			$total_count = $total['cnt'];
			$total_page  = ceil($total_count / $rows);  // 전체 페이지 계산
